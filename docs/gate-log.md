...
## 補記（3A 之後）：R2 自訂網域與公開註冊

**日期：** 2026-08-11

### 公開註冊已關閉

`GOTRUE_DISABLE_SIGNUP=true` 生效，實測確認：

```text
POST /auth/v1/signup → 422 signup_disabled
```

先前是 500（沒設 SMTP 導致確認信失敗而回滾）——那是「剛好失敗」，
現在是「刻意拒絕」。差別在於接上 SMTP 之後前者會失效，後者不會。

### R2 改用自訂網域 1page-r2.snowrealm.pet

**兩個網域必須都認得。** 只切換到新網域會讓既有媒體記錄的網址
無法還原 key，被當成外部網址而在畫面上整批消失。
網域搬遷是加法，不是替換。

```text
NEXT_PUBLIC_R2_PUBLIC_DOMAIN_URL   自訂網域（新上傳使用）
NEXT_PUBLIC_R2_PUBLIC_URL          r2.dev（保留，既有記錄使用）
```

`keyFromPublicUrl` 比對所有已知網域；`next.config` 的 `remotePatterns`
同樣兩個都列。已加測試守住這條不變量。

環境變數容許不帶 scheme——設定介面上常常只填主機名，
`new URL()` 會直接拋錯。程式自動補 `https://`。
...
> 不是「這個功能本身安全」。若哪天換成自訂網域，
> 同註冊網域下的風險會立刻回來，而那時不會有人記得這件事。

現在它回來了：`1page-r2.snowrealm.pet` 與 `1page.snowrealm.pet`
同屬 `snowrealm.pet`。

目前 Supabase 的 auth cookie 是 host-only，因此尚未實際暴露——
但那又是一個「剛好安全」。結論不變且更堅定：
**在有伺服器端 sanitizer 之前，SVG 不進白名單。**

註解已更新，把「這是當初預告過的情況」寫進去，
避免下一個人重新推導一次。
...
| # | 項目 | 結果 |
|---|---|---|
| 1–4 | typecheck / lint / test / build | ✅ 153 unit tests（3A 140 → 3B 153） |
| 5 | e2e + 截圖 | ✅ 98/98、48 張（新增 `/_dev/theme`） |

### 出口條件核對

| 出口條件 | 驗證方式 | 結果 |
|---|---|---|
| `--site-*` 絕不出現在 `:root` | e2e 於真實瀏覽器讀 `getComputedStyle(document.documentElement)` | ✅ |
| 切換主題不影響官網品牌色 | e2e 斷言 `--color-brand-*` 仍為 tokens.css 的值 | ✅ |
| token 契約測試涵蓋此規則 | 單元測試驗證產生的變數名稱全在 `--site-*` 命名空間 | ✅ |

單元測試只能證明「產生的名稱對」，證明不了「瀏覽器算出來的樣式是隔離的」——
CSS 的繼承與層疊只有在真的瀏覽器裡才看得準，因此兩層都要有。

### 注入點的第二道防線

3A 的 schema 已經擋掉含 `;` `{` `}` 的色彩值。`theme.ts` 再擋一次是刻意的重複：
...
SSR 會把 inline style 序列化成 `style="--site-accent: red"` 屬性；
值裡若有分號就會變成同一個 style 屬性內的**額外宣告**——那是真正的 CSS 注入。

兩層的價值在於彼此不互相假設：若有人繞過 schema 直接呼叫渲染
（測試、腳本、未來的新資料來源），這一層仍在。

不安全的值**只略過該項**而非整個主題：少一個顏色會退回容器預設值，
整個主題失效則是整片空白。失敗要盡量小。

### ⚠️ 守衛本身有漏洞，而且漏的正是要守的東西

`no-hardcoded-design-values` 的 inline style 規則原本比對 `style={{`——
只抓得到物件字面值。`SiteScope` 寫的是 `style={vars}`，**整個漏掉**。

也就是說，那條規則從 1A 立起到現在，對「主題注入」這種真正需要被審視的寫法
完全無效，而它看起來一直是綠的。

已改為比對 `style={`，並補上三處具名例外（進度條、SiteScope、主題驗證頁）。

> 守衛通過不等於守衛有效。這是這個專案第二次遇到同型問題——
> 第一次是 2E 靜默跳過的安全測試。

### 1C 立下的容器不變量差點被自己破壞

1C 在 `TemplateExperienceShell` 放了空的 `[data-site-scope]` 容器，
理由是「即使 Preview 是張 stub，容器也要先存在，避免 Phase 3 忘記 scope 這件事」。

3B 改寫時，我把「沒有 config 就不渲染容器」寫了進去——
1C 的測試立刻失敗。那條測試擋下的正是它當初被寫下來要擋的事。
已恢復：無 config 時仍保留容器，只是不注入任何變數。

### `/_dev/theme` 驗證三件事

```text
1. 同一份標記在不同 theme 下呈現不同外觀
2. 兩個 scope 並排互不干擾、巢狀由內層完全覆蓋
3. 官網品牌色完全不受影響  ← 最容易出錯也最難察覺的一項
```
