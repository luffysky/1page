...
> V1.7 變更（CR-007）：CRM 設計器可從 Excel／CSV 匯入結構與資料。影響 CR-003-5 的範圍。
>
...
## CR-007 — CRM 設計器可從 Excel／CSV 匯入

| 項目 | 內容 |
|---|---|
| 日期 | 2026-08-18 |
| 影響章節 | CR-003-5（前台 CRM 設計器）的範圍 |
| 原規格 | CRM 的結構一個欄位一個欄位手動設計；記錄一次填一筆 |
| 變更為 | 可上傳 .xlsx／.csv：從欄位猜型別建出一類，以及批次匯入記錄 |
| 裁決 | Luffy 指示採用 |
| 版本 | V1.6 → V1.7 |

**動機（Luffy 原話）：**「CRM設計加上匯入excel csv功能 值的類別看有沒有辦法
正確判斷或者讓使用者後面可以修改 填資料也可以匯入excel或csv」

### 界線

```text
做            讀第一張工作表、猜欄位型別、讓使用者逐欄改、批次寫入記錄
不做          寫出 Excel（匯出）、公式、多張工作表、樣式、
              舊版 .xls（2007 以前，格式完全不同）
不做          猜編碼。UTF-8 以外一律當 UTF-8 讀
```

### 三條隨此 CR 生效的硬性要求

1. **猜測是草稿，不是既成事實。** 每一欄都必須在畫面上顯示
   猜出來的型別、**為什麼這樣猜**、以及實際的幾個值，而且型別可以當場改。
   把握低的要看得出來——全部長一樣的話使用者只會一路按到底。

2. **不得安靜地改變值。** 電話這種有前導零的一律當文字
   （存成數字會掉開頭的 0，而它看起來完全正常）；
   斜線日期要轉成 `YYYY-MM-DD` 才存（不轉的話畫面說是日期、每一列卻都匯不進去）；
   轉不動的原樣送進 schema 擋下來，不在中間吞掉。

3. **一列壞掉不拖垮整份，也不安靜跳過。** 匯入其餘的，並且逐列指名
   第幾列、哪一欄、哪裡不對——而且必須在**按下送出之前**就看得到。
   列號要與使用者在 Excel 裡看到的一致。

### 不新增執行期依賴

`.xlsx` 用瀏覽器原生的 `DecompressionStream` 讀（ZIP + 兩份 XML），
CSV 用自己寫的 RFC 4180 解析器。整個專案只有十個執行期依賴，
而最常見的那個 xlsx 套件在 npm 上已標為 deprecated 且有過 CVE。

**檔案在瀏覽器裡讀，不上傳。** 送到伺服器的只有對應好之後的那幾列值，
而伺服器用與手動新增**同一份** `recordSchemaFor` 逐筆重驗。
